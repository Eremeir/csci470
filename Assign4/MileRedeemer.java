...
public class MileRedeemer
{
    ...
    private void upgradeTickets(ArrayList<String> tickets)
    {
        for (int i = 0; i < tickets.size(); i++)
        {
            ...
            if (remainingMiles >= destination.getFirstClassUpgrade())
            {
                tickets.set(j, createTicketDesc(destination, "First Class"));
    ...
    private int getCityIndex(String ticket)
    {
        ...
        for (int i = 0; i < destinationArray.length; i++)
        {
            if (destinationArray[i].getCity().equals(destinationCity))
            {
                return i;
            }
        }
